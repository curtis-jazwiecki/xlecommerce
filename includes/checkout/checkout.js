			...
			if ($(this).attr('name') != undefined && $(this).attr('type') != 'checkbox'){
				...
				var processAddressFunction = function (){
					...
					if ($(this).hasClass('required')){
		                ...
		                $('select[name="shipping_country"], input[name="shipping_street_address"], input[name="shipping_zipcode"], *[name="delivery_state"]').each(function (){
                         if (checkoutClass.fieldErrorCheck($(this), false, true) == false){
...
//				$element1=null;
	...
	updateModuleMethods: function (action, noOrdertotalUpdate){
		...
		this.queueAjaxRequest({
			...
			success: function (data){
                                ...
                                //    $('#no' + descText + 'Address').css('display', '');

                                //    $('#' + action + 'Methods').html('').css('display', 'none');
	...
	setModuleMethod: function (type, method, successFunction){
		...
		var checkoutClass = this;
                ...
                if(method!=undefined && method.indexOf('&')!=-1){

                    params = 'action=set' + (type == 'shipping' ? 'Shipping' : 'Payment') + 'Method&' + method;

                } else {

                    params = 'action=set' + (type == 'shipping' ? 'Shipping' : 'Payment') + 'Method&method=' + method;
    ...
    setShippingMethod: function ($button){
			...
			if (1==1){
                ...
                $('input:radio[name!="shipping"]').each(function(){
                    ...
                    vendor_id = $(this).attr('name').substring($(this).attr('name').indexOf('_')+1);

                    if ($('input[name="products_' + vendor_id + '"]').val()!=undefined){

                        if($(this).attr('checked')){

                            ship_methods += $(this).attr('name') + '=' +  $(this).val() + '&products_' + vendor_id + '=' + $('input[name="products_' + vendor_id + '"]').val() + '&';
                ...
                if (ship_methods!=''){
                    ...
                    ship_methods = ship_methods.substring(0, ship_methods.length-1);

                }

                //alert("shipping methods got"+ship_methods);
				this.setModuleMethod('shipping', ship_methods, function (data){});
		...
		if (typeof(data.id) !="undefined"){		
                ...
                selectedM = $("#"+data.id);
              	...
              	$('.pay1').each(function(){
                    ...
                    if($(this).parent().attr('id')!=selectedM.attr('id'))
	...
	processBillingAddress: function (skipUpdateTotals){
		...
		if(skipUpdateTotals != true && ($('#diffShipping:checked').size() <= 0  && this.loggedIn != true))
	...
	processShippingAddress: function (skipUpdateTotals){
		...
		$('select[name="shipping_country"], input[name="shipping_street_address"], input[name="shipping_zipcode"], input[name="shipping_city"], select[name="delivery_state"],input[name="delivery_state"]').each(function (){
	...
	//}
		...
		*/
				...
				success: function (){
					...
					checkoutClass.updateOrderTotals();
					 ...
					 isTab = true;
			                    ...
			                    $('input[name="gv_redeem_code"]').focus();
...

			{
				...
				if ($(':radio[name="payment"]:checked').length <= 0){
					...
					if ($(':radio[name="payment"]').length>0){		
                        ...
                        //var functionName = 'jsValidation'+$('input[name=payment]').val()+' ()';
		...
		$('#checkoutButton').click(function() {
			...
			if ($('input[name="agreetotermsconditions"]').length){

				if (!$('input[name="agreetotermsconditions"]').attr('checked')){

					alert("please agree to terms & conditions before proceeding further.")
			...
			//return checkAllErrors();
                        ...
                        if (checkAllErrors()) {
                            ...
                            var checkoutPostFields = 'action=process';

                            $('input[type=text]').each(function(){

                                if ($(this).attr('name').substring(0, 3)  !== 'qty'){ 

                                    checkoutPostFields = checkoutPostFields + '&'+$(this).attr('name')+'='+$(this).val();
                            ...
                            checkoutClass.queueAjaxRequest({
                                ...
                                data: checkoutPostFields,

                                type: 'post',

                                dataType: 'html',

                                beforeSendMsg: 'Processing...', 

                                success: function(data){

                                    $('#checkoutContainerMessage').html(data);
				...
				checkoutClass.queueAjaxRequest({
					...
					success: function (data){
                                            ...
                                            //url = $('form[name="redirectForm"]').attr('action');
                                            ...
                                            //   data: $('form[name="redirectForm"]').serialize()

                                            //});

					},

					errorMsg: 'There was an error processing request.'
