...
if ($sts->display_template_output) {
  $sts->restart_capture (); // Capture header from the actual script, needed to take out javascript
} 
...
switch (true) {
  ...
  // PRODUCT_INFO.PHP
  case (( strstr($_SERVER['PHP_SELF'],FILENAME_PRODUCT_INFO) or strstr($PHP_SELF,FILENAME_PRODUCT_INFO)) && (isset($HTTP_GET_VARS['products_id']) && $HTTP_GET_VARS['products_id'] != '') ):
...
// advanced_search_result.php
  case (strstr($_SERVER['PHP_SELF'],FILENAME_ADVANCED_SEARCH_RESULT) or strstr($PHP_SELF, FILENAME_ADVANCED_SEARCH_RESULT));
    $tags_array = tep_header_tag_page(HTTA_ADVANCED_SEARCH_RESULT_ON, HEAD_TITLE_TAG_ADVANCED_SEARCH_RESULT, 
                                      HTDA_ADVANCED_SEARCH_RESULT_ON, HEAD_DESC_TAG_ADVANCED_SEARCH_RESULT, 
                                      HTKA_ADVANCED_SEARCH_RESULT_ON, HEAD_KEY_TAG_ADVANCED_SEARCH_RESULT );
   break;

// information.php
  case (strstr($_SERVER['PHP_SELF'],FILENAME_INFORMATION) or strstr($PHP_SELF, FILENAME_INFORMATION));
    $tags_array = tep_header_tag_page(HTTA_INFORMATION_ON, HEAD_TITLE_TAG_INFORMATION, 
                                      HTDA_INFORMATION_ON, HEAD_DESC_TAG_INFORMATION, 
                                      HTKA_INFORMATION_ON, HEAD_KEY_TAG_INFORMATION );
   break;

// shop_by_manufacturer.php
  case (strstr($_SERVER['PHP_SELF'],FILENAME_SHOP_BY_MANUFACTURER) or strstr($PHP_SELF, FILENAME_SHOP_BY_MANUFACTURER));
...
// Get Meta Tags For Info Pages
	$meta_check_query = tep_db_query("select information_title, information_meta_title, information_meta_tag, information_meta_tag_keywords from information where information_id = '".$_GET['info_id']."'");
	$meta_check = tep_db_fetch_array($meta_check_query);

// End Meta Tags For Info Pages

if($_SERVER['PHP_SELF'] == '/information.php')
  {
        if(!empty($meta_check['information_meta_title'])){
            echo '  <title>' . $meta_check['information_meta_title'] . '</title>' . "\n";
        }elseif(!empty ($meta_check['information_title'])){
            echo '  <title>' . $meta_check['information_title'] . '</title>' . "\n";
        }else{
            echo '  <title>' . $tags_array['title'] . '</title>' . "\n";
        }
	echo '<meta name="description" content="'.$meta_check['information_meta_tag'].'" />'."\n";
	echo '<meta name="keywords" content="'.$meta_check['information_meta_tag_keywords'].'" />'."\n";
  }
else
  {
        echo '  <title>' . $tags_array['title'] . '</title>' . "\n";
