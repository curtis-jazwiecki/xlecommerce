...
if(FEATURED_PRODUCTS_DISPLAY == 'true'){
	...
	if (tep_db_num_rows($featured_products_query)){
		...
		if (MODULE_STS_DEFAULT_STATUS=='true' && MODULE_STS_TEMPLATE_FOLDER!='' && ( ( ( !isset($on_home_page) || !$on_home_page ) && file_exists(DIR_FS_CATALOG . DIR_WS_INCLUDES . 'sts_templates/' . MODULE_STS_TEMPLATE_FOLDER . '/blocks/infobox_06.php.html') ) || ( $on_home_page && file_exists(DIR_FS_CATALOG . DIR_WS_INCLUDES . 'sts_templates/' . MODULE_STS_TEMPLATE_FOLDER . '/blocks/featured_products.php.html')  ) ) ) {	
			...
			if (empty($custom)){
				...
				$content = file_get_contents(DIR_FS_CATALOG . DIR_WS_INCLUDES . 'sts_templates/' . MODULE_STS_TEMPLATE_FOLDER . '/blocks/featured_products.php.html');
			...
			while ($featured_products = tep_db_fetch_array($featured_products_query)) {
				...
				$price = $currencies->display_price($featured_products['products_price'], tep_get_tax_rate($featured_products['products_tax_class_id']));
				...
				if (!empty($items_per_row) && $items_per_row>1){
					...
					$temp = str_ireplace( array('$image_' . $count, '$link_' . $count, '$name_' . $count, '$price_' . $count), array($image, $link, $name, $price), $temp);
			...
			while ($featured_products = tep_db_fetch_array($featured_products_query)) {
				...
				if($featured_products['specstat'] && $featured_products['products_price'] > $featured_products['specials_new_products_price']) {
