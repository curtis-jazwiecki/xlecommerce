  ...
  class usps {
     ...
     function quote($method = '', $module = '', $vendors_id = '1', $products_qty_for_vendor = null) {
      ...
      for ($i = 0; $i < $PackageSize; $i++) {
        ...
        if ($order->delivery['country']['iso_code_2'] == 'US') {
          if (tep_not_null($Package['SpecialServices']['SpecialService']))
            foreach ($Package['SpecialServices']['SpecialService'] as $key => $val)
              if (isset ($dExtras[$val['ServiceName']]) && tep_not_null($dExtras[$val['ServiceName']]) && ((constant('MODULE_SHIPPING_USPS_RATE_TYPE_' . $vendors_id) == 'Online' && $val['AvailableOnline'] == 'true') || (cosnatnt('MODULE_SHIPPING_USPS_RATE_TYPE_' . $vendors_id) == 'Retail' && $val['Available'] == 'true'))) {
          ...
          foreach ($Package['ExtraServices']['ExtraService'] as $key => $val) {
            if ( is_array ( $val ) && 
                 ...
                 ((constant('MODULE_SHIPPING_USPS_RATE_TYPE_' . $vendors_id) == 'Online' && 
                     $val['AvailableOnline'] == 'True') || 
                   (constant('MODULE_SHIPPING_USPS_RATE_TYPE_' . $vendors_id) == 'Retail' && 
                     $val['Available'] == 'True'))) {
        ...
        foreach ($Services as $key => $val) {
          ...
          if ($sDisplay == 'H')
            $hiddenServices[] = array (
              $Services[$key]['ServiceName'] => (constant('MODULE_SHIPPING_USPS_RATE_TYPE_' . $vendors_id) == 'Online' ? $Services[$key]['PriceOnline'] : $Services[$key]['Price'])
            );
        ...
        if (sizeof($shownServices) > 0) {
          ...
          '<div style="padding-left:20px; float:left;">' . constant('MODULE_SHIPPING_USPS_TEXT_BASE_COST_' . $vendors_id) . '</div><div style="padding-right:20px; float:right;">' . ($cost == 0 ? MODULE_SHIPPING_USPS_TEXT_FREE : $currencies->format(($cost + (sizeof($hiddenServices) > 0 ? $handling + $hiddenCost : 0)) * $shipping_num_boxes)) . '</div><div style="clear:both;"></div>';
          if (sizeof($hiddenServices) == 0) {
            $shownString .= '<div style="padding-left:20px; float:left;">' . constant('MODULE_SHIPPING_USPS_TEXT_HANDLING_COST_' . $vendors_id) . '</div>' .
              '<div style="padding-right:20px; float:right;">' . ($handling == 0 ? constant('MODULE_SHIPPING_USPS_TEXT_FREE_' . $vendors_id) : $currencies->format($handling * $shipping_num_boxes)) . '</div>' .
          ...
          foreach ($shownServices as $key => $val) {
            foreach ($shownServices[$key] as $key1 => $val1) {
              $shownString .= '<div style="padding-left:20px; float:left;">' . str_replace(array (
                ...
                '&reg;',
                '&trade;'
              ), $key1) . '</div><div style="padding-right:20px; float:right;">' . ($val1 == 0 ? constant('MODULE_SHIPPING_USPS_TEXT_FREE_' . $vendors_id) : $currencies->format($val1 * $shipping_num_boxes)) . '</div><div style="clear:both;"></div>';
      ...
      $weight_text = constant('MODULE_SHIPPING_USPS_WEIGHTS_' . $vendors_id) == 'True' ? ' ' . $this->pounds . ' lbs, ' . $this->ounces . ' oz' : '';
      $this->quotes = array (
        'id' => $this->code,
    ...
    function _getQuote($vendors_id = '1') {
      global $order;
        ...
        $vendors_data_query = tep_db_query("select vendor_country, vendors_zipcode from " . TABLE_VENDORS . " where vendors_id = '" . (int) $vendors_id . "'");
      ...
      if ($order->delivery['country']['iso_code_2'] == 'US') {
        ...
        $request = '<RateV4Request USERID="' . constant('MODULE_SHIPPING_USPS_USERID_' . $vendors_id) . '">' . '<Revision>2</Revision>';
        $package_count = 0;
        $service = '';
    ...
    function install($vendors_id = '1') {
      ...
      tep_db_query("insert into " . TABLE_VENDOR_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added, vendors_id) values ('Shipping Methods (Domestic and International)',  'MODULE_SHIPPING_USPS_TYPES_" . $vendors_id . "', '0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00, 0, 70, 0.00', '<b><u>Checkbox:</u></b> Select the services to be offered<br><b><u>Minimum Weight (lbs)</u></b>first input field<br><b><u>Maximum Weight (lbs):</u></b>second input field<br><br>USPS returns methods based on cart weights.  These settings will allow further control (particularly helpful for flat rate methods) but will not override USPS limits', '6', '0', 'tep_cfg_usps_services(array(" . $this->get_usps_services_list() . "), ', now(), '" . $vendors_id . "')");
      ...
      tep_db_query("insert into " . TABLE_VENDOR_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added, vendors_id) values ('Show Weights', 'MODULE_SHIPPING_USPS_WEIGHTS_" . $vendors_id . "', 'True', 'Displays the package weight on the quotes.', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now(), '" . $vendors_id . "')");
    //  tep_db_query("insert into " . TABLE_VENDOR_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('USPS Server', 'MODULE_SHIPPING_USPS_SERVER_" . $vendors_id . "', '', 'The USPS server to send the request to. <b>Change this only if instructed to do so!</b>', '6', '0', now())");
      tep_db_query("insert into " . TABLE_VENDOR_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added, vendors_id) values ('Send Debug Email', 'MODULE_SHIPPING_USPS_DEBUG_" . $vendors_id . "', 'False', 'Send an email to the store owner with the USPS request and response.', '6', '0', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now(), '" . $vendors_id . "')");
    }
    ...
    function keys($vendors_id = '1') {
      return array (
        ...
        'MODULE_SHIPPING_USPS_WEIGHTS_' . $vendors_id,
       // 'MODULE_SHIPPING_USPS_SERVER_' . $vendors_id,
        'MODULE_SHIPPING_USPS_DEBUG_' . $vendors_id
      );
    }
