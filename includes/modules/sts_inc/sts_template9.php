...
$sts->blocks = array();
foreach($template9_blocks as $block){
    if (is_array($block)){
        ...
        if (!$is_enabled){
            ...
            switch( strtolower( $block['type'] ) ){
                case 'related':
                    ...
                    if (isset($_GET['products_id']) && !empty($_GET['products_id'])){
                        $temp_file = 't9_xsell_products-' . $language . '.cache' . $_GET['products_id'];
                        if (USE_CACHE=='true'){
                ...
                case 'featured_categories':
                    $val = '';
                    if (isset($_GET['products_id']) && !empty($_GET['products_id'])){
                        $temp_file = 't9_featured_categories-' . $language . '.cache' . $_GET['products_id'];
                        if (USE_CACHE=='true'){
                            if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
                ...
                case 'featured_manufacturers':
                    $val = '';
                    if (isset($_GET['products_id']) && !empty($_GET['products_id'])){
                        $temp_file = 't9_featured_manufacturers-' . $language . '.cache' . $_GET['products_id'];
                        if (USE_CACHE=='true'){
                            if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
                ...
                case 'hot':
                    $val = '';
                    $temp_file = 't9_hot_products-' . $language . '.cache';
                    if (USE_CACHE=='true'){
                        if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
                ...
                case 'recommended':
                    $val = '';
                    if (isset($_GET['products_id']) && !empty($_GET['products_id'])){
                        $temp_file = 't9_recommended_products-' . $language . '.cache' . $_GET['products_id'];
                        if (USE_CACHE=='true'){
                            if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
         ...
         case 'featuredgroup3':
                ...
                case 'popular':
                    ...
                    $val = '';
                    $temp_file = 't9_popular_products-' . $language . '.cache';
                    if (USE_CACHE=='true'){
                        if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
		...
		if (USE_CACHE=='true'){
            $temp_file = 't9_' . $block . '-' . $language . '.cache';
            if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
                $val = get_block_content('template9', $block);
                write_cache($val, $temp_file);
            }
        } else {
            $val = get_block_content('template9', $block);
