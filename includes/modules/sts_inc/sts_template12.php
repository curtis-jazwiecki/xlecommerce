...
$template12_blocks = array(
	...
	'shopping_cart', 
	'navigation_menu_top',
	'alphabetical_brand_index', 
	'slider',
	'top_box_right_to_slider',
	'medium_box_right_to_slider',
	'bottom_box_right_to_slider',
	'newsletter_signup_block',
		...
		//'enabled'=> ENABLE_POPULAR_PRODUCTS=='True' ? true : false, 
                'enabled' => true, 
		'parameters'=>array('custom'=>true, 'on_home_page'=>true, 'max_count'=>'6', 'items_per_row'=>'2'),  
	),
	'box_left_to_featured_products', 
	'brand_block', 
	'promo_block', 
	'customer_reviews', 
	'footer_box_01',
...
$sts->blocks = array();
foreach($template12_blocks as $block){
	if (is_array($block)){
		...
		if (!$is_enabled){
			...
			switch( strtolower( $block['type'] ) ){
				case 'related':
                                    ...
                                    if (isset($_GET['products_id']) && !empty($_GET['products_id'])){
                                        $temp_file = 't12_xsell_products-' . $language . '.cache' . $_GET['products_id'];
                                        if (USE_CACHE=='true'){
				...
				case 'featured_categories':
                                    $val = '';
                                    if (isset($_GET['products_id']) && !empty($_GET['products_id'])){
                                        $temp_file = 't12_featured_categories-' . $language . '.cache' . $_GET['products_id'];
                                        if (USE_CACHE=='true'){
                                            if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
				...
				case 'featured_manufacturers':
                                    $val = '';
                                    if (isset($_GET['products_id']) && !empty($_GET['products_id'])){
                                        $temp_file = 't12_featured_manufacturers-' . $language . '.cache' . $_GET['products_id'];
                                        if (USE_CACHE=='true'){
                                            if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
				...
				case 'hot':
                                    $val = '';
                                    $temp_file = 't12_hot_products-' . $language . '.cache';
                                    if (USE_CACHE=='true'){
                                        if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
				...
				case 'recommended':
                                    $val = '';
                                    /*if (isset($_GET['products_id']) && !empty($_GET['products_id'])){
                                        $temp_file = 't12_recommended_products-' . $language . '.cache' . $_GET['products_id'];
                                        if (USE_CACHE=='true'){
                                            if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
				...
				case 'popular':
                                    $val = '';
                                    $temp_file = 't12_popular_products-' . $language . '.cache';
                                    /*if (USE_CACHE=='true'){
                                        if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
            ...
            //$sts->template[$block] = get_block_content('template12', $block);
            $val = '';
            if ($block=='slider'){
                $val = get_block_content('template12', $block);
            } else {
                if (USE_CACHE=='true'){
                    $temp_file = 't12_' . $block . '-' . $language . '.cache';
                    if (!read_cache($val, $temp_file, PURGE_CACHE_DAYS_LIMIT)){
                        $val = get_block_content('template12', $block);
                        write_cache($val, $temp_file);
                    }
