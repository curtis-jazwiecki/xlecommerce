...
$product_info_query = tep_db_query("select p.products_id, pd.products_name, pd.products_description, p.products_model, p.products_quantity, p.products_image, p.products_mediumimage, p.products_largeimage, pd.products_url, p.products_price, p.products_tax_class_id, p.products_date_added, p.products_date_available, p.manufacturers_id, pe.upc_ean, pe.unit_msrp, pe.min_acceptable_price from " . TABLE_PRODUCTS . " p inner join " . TABLE_PRODUCTS_DESCRIPTION . " pd on (pd.products_id = p.products_id and pd.language_id='" . (int)$languages_id . "') left join  products_extended pe on p.products_id=pe.osc_products_id where p.products_status = '1' and p.products_id = '" . $products_id . "'");
$product_info = tep_db_fetch_array($product_info_query);
	
$template_pinfo['upc'] = !empty($product_info['upc_ean']) ? $product_info['upc_ean'] : '';
$template_pinfo['map'] = !empty($product_info['min_acceptable_price']) ? $product_info['min_acceptable_price'] : '';
$template_pinfo['msrp'] = !empty($product_info['unit_msrp']) ? $product_info['unit_msrp'] : '';
...
if (tep_not_null($product_info['products_image'])) {
    ...
    $template_pinfo['imagemedium'] = tep_image(DIR_WS_IMAGES . $product_info['products_mediumimage'], addslashes($product_info['products_name']), '','','');
...
if (tep_not_null($product_info['products_largeimage'])) {
    //$template_pinfo['urllargeimage'] = (($request_type == 'SSL') ? HTTP_SERVER : HTTPS_SERVER) . (($request_type == 'SSL') ? DIR_WS_HTTPS_CATALOG : DIR_WS_HTTP_CATALOG) . DIR_WS_IMAGES . $product_info['products_largeimage'];
    $template_pinfo['urllargeimage'] = $product_info['products_largeimage'];
} else {
    $template_pinfo['urllargeimage'] = '';
}
