...
if(FEATURED_PRODUCTS_DISPLAY == 'true'){
	...
	/*if (file_exists(DIR_FS_CATALOG . DIR_WS_IMAGES. 'featured_header.jpg')) {
		...
		$heading = '<a class="headerNavigation" href="' . tep_href_link(FILENAME_FEATURED_PRODUCTS) . '">' .TWO_FEATURED_PRODUCT_TITLE . '</a>';
	//}
 ...
 $total_featured = tep_db_num_rows($featured_products_query);
	...
	if (tep_db_num_rows($featured_products_query)){
		if (MODULE_STS_DEFAULT_STATUS=='true' && MODULE_STS_TEMPLATE_FOLDER!='' && ( ( ( !isset($on_home_page) || !$on_home_page ) && file_exists(DIR_FS_CATALOG . DIR_WS_INCLUDES . 'sts_templates/' . MODULE_STS_TEMPLATE_FOLDER . '/blocks/infobox_06.php.html') ) || ( $on_home_page && file_exists(DIR_FS_CATALOG . DIR_WS_INCLUDES . 'sts_templates/' . MODULE_STS_TEMPLATE_FOLDER . '/blocks/featured_products2.php.html')  ) ) ) {	
			if ($custom){
                ...
                $content = file_get_contents(DIR_FS_CATALOG . DIR_WS_INCLUDES . 'sts_templates/' . MODULE_STS_TEMPLATE_FOLDER . '/blocks/featured_products2.php.html');
			}
