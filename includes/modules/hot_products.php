...
$hot_query = tep_db_query("select p.products_id, p.products_image, p.products_mediumimage, pd.products_name, p.products_tax_class_id, p.products_price from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = pd.products_id and pd.language_id = '" . $languages_id . "' and p.products_status = '1' ORDER BY p.products_ordered desc limit ".MAX_DISPLAY_HOT_PRODUCT); //ORDER BY p.reviews_rating DESC 
// EOF Separate Pricing Per Customer
$num_hot_products = tep_db_num_rows($hot_query); 
// EOF Separate Pricing Per Customer
//$num_products_xsell = tep_db_num_rows($hot_products_query);
if ($num_hot_products > 0) {	
    if (MODULE_STS_DEFAULT_STATUS=='true' && MODULE_STS_TEMPLATE_FOLDER!='' && file_exists(DIR_FS_CATALOG . DIR_WS_INCLUDES . 'sts_templates/' . MODULE_STS_TEMPLATE_FOLDER . '/blocks/infobox_04.php.html') ){		
        $content = file_get_contents(DIR_FS_CATALOG . DIR_WS_INCLUDES . 'sts_templates/' . MODULE_STS_TEMPLATE_FOLDER . '/blocks/infobox_04.php.html');		
        $output = '';		
        ...
        if ($header_bof!==false && $header_eof!==false){			
            ...
            $header_content = str_ireplace('$header', 'Hot Products', $header_content);		
            } else {			
                $header_exists = false;			
                $header = '';			
                $header_content = '';		
        ...
        while ($hot_products = tep_db_fetch_array($hot_query)) {
            $hot_products['products_image'] = (tep_not_null($hot_products['products_mediumimage']) ? $hot_products['products_mediumimage'] : $hot_products['products_image']);
            ...
            if ($hot_products['specials_new_products_price']) {				
                $price =  '<s>' . $currencies->display_price($hot_products['products_price'], tep_get_tax_rate($hot_products['products_tax_class_id'])) . '</s><br>';				
                $price .= '<span class="productSpecialPrice">' . $currencies->display_price($hot_products['specials_new_products_price'], tep_get_tax_rate($hot_products['products_tax_class_id'])) . '</span>';			
                } else {				
                    $price =  $currencies->display_price($hot_products['products_price'], tep_get_tax_rate($hot_products['products_tax_class_id']));			
                    }   			
                if (tep_not_null($hot_products['products_image'])) {				
                    ...
                    $link = tep_href_link(FILENAME_PRODUCT_INFO, 'products_id=' . $hot_products['products_id']);			
                    $name = $hot_products['products_name'];
					//code added to show manufacturer on products on homepage(Template21)
                    $manufacturer ="";
                $manu_link="";
				$manufacturer_query = tep_db_query("select m.manufacturers_name,m.manufacturers_id from ". TABLE_PRODUCTS ." p ,".TABLE_MANUFACTURERS." m where p.manufacturers_id= m.manufacturers_id and p.products_id =". $hot_products['products_id']);
                while($manufacturer_row = tep_db_fetch_array($manufacturer_query)){
                    $manufacturer = $manufacturer_row['manufacturers_name'];
                    $manu_link = tep_href_link('shop.php','manufacturers_id='.$manufacturer_row['manufacturers_id']); 
                }
                $msrp_price="";
                $msrp_query = tep_db_query("select unit_msrp from products_extended where osc_products_id =". $hot_products['products_id']);                 while($msrp_row = tep_db_fetch_array($msrp_query)){
                    ...
                    $entry = str_ireplace(array('$image', '$link', '$name', '$price','$manufacturer','$manu_link','$msrp_price'), array($image, $link, $name, $price,$manufacturer,$manu_link,$msrp_price),$block_content);	
                    //code ends 	
                    $output .= $entry;					
                    }				
                    $entry = str_ireplace(array('$image', '$link', '$name', '$price','$manufacturer','$manu_link','$msrp_price'), array($image, $link, $name, $price,$manufacturer,$manu_link,$msrp_price),$block_content);		
