...
if(is_array($_GET) && !empty($_GET)){
    foreach($_GET as $key => $val){
        if(strstr($key, 'amp;') !== false){
            $nkey = str_replace('amp;', '', $key);
            $_GET[$nkey] = $val;
            $HTTP_GET_VARS[$nkey] = $val;
        }
    }
}
// #06 9Jan2014 (MA) EOF
...
//#14 13Jan2014 (MA) BOF  
  ...
  $blacklisted_ip_str = IP_BLACKLISTED;
  if (!empty($blacklisted_ip_str)){
    if (!empty($ip_address)){
        list($cp1, $cp2, $cp3, $cp4) = explode('.', $ip_address); 
        $fraud_ip_addresses = explode(',', $blacklisted_ip_str);
        foreach($fraud_ip_addresses as $ip){
            ...
            if ($p1==$cp1 || $p1=='*'){
                if ($p2==$cp2 || $p2=='*'){
                    if ($p3==$cp3 || $p3=='*'){
                        if ($p4==$cp4 || $p4=='*'){
                            tep_redirect(tep_href_link('blacklisted.html'));
                            break;
                        }
                    }
                }
...
// check if sessions are supported, otherwise use the php3 compatible session class
  if (!function_exists('session_start')) {
    define('PHP_SESSION_NAME', 'OBNsid');
    ...
    switch ($HTTP_GET_VARS['action']) {
        case 'newsletter_signup':
            ...
            if (!empty($name) && !empty($email)){
                $is_registered = '0';
                $customer_query = tep_db_query("select customers_id from customers where customers_email_address='" . tep_db_prepare_input($email) . "'");
		...
		//BOF:MVS correction
			...
			foreach($_POST as $key => $value){
				if (strpos($key, 'products_')!==false){
					for ($i=0, $n=sizeof($_POST[$key]); $i<$n; $i++) {
						if (in_array($_POST[$key][$i], (is_array($_POST['cart_delete']) ? $_POST['cart_delete'] : array()))) {
							$cart->remove($_POST[$key][$i]);
						} else {
							$attributes = ($_POST['id'][$_POST[$key][$i]]) ? $_POST['id'][$_POST[$key][$i]] : '';
							$cart->add_cart($_POST[$key][$i], $_POST['cart_quantity'][$i], $attributes, false);
						}
					}
				}
			}
      ...
      case 'buy_now' :        if (isset($HTTP_GET_VARS['products_id'])) {
                                if (tep_has_product_attributes($HTTP_GET_VARS['products_id']) || (tep_required_disclaimer($HTTP_GET_VARS['products_id']) && $_GET['disclaimer_'.$_GET['products_id']] != '1' )) {
...
if (basename($PHP_SELF) != FILENAME_INDEX) {
