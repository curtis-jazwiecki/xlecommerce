...
// #################### End Added CGV JONYO ######################
 ...
 if ($GLOBALS[$class]->enabled && $GLOBALS[$class]->member_class) {
   $member_string = $GLOBALS[$class]->membership_selection();
   if ($member_string != '') {
              $output_string2 .=  '<tr colspan="4"><td colspan="4" width="100%">' .  tep_draw_separator('pixel_trans.gif', '100%', '10') . '</td></tr>';

              $output_string2 .= $member_string;
            }         
        }
      }
        if ($output_string != '') {
         $output_string = $header_string . $output_string . $output_string2;
