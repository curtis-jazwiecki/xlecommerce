...
function is_xml_feed_product($product_id) {
	$check_query = tep_db_query("select count(*) as total from products_extended where osc_products_id = '" . (int)$product_id . "' and xml_feed_id <> 0 ");
...
//EOF:fraud_prevention
  ...
  $spec_name_query = tep_db_query("select distinct(specification_id), psv.value, psn.name, psn.id as name_id from product_specifications ps left join product_specification_values psv on ps.specification_id=psv.id left join product_specification_names psn on psv.specification_name_id=psn.id inner join products p on ps.products_id=p.products_id left join manufacturers m on p.manufacturers_id=m.manufacturers_id  "  . (!empty($price_str) ? " left join " . TABLE_SPECIALS_RETAIL_PRICES . " s on p.products_id = s.products_id ":"") .  (!empty($keywords_str) ? " inner join products_description pd on p.products_id = pd.products_id and pd.language_id='1'" : "") . " inner join " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c on p.products_id=p2c.products_id inner join " . TABLE_CATEGORIES . " c on p2c.categories_id=c.categories_id where 1=1 and c.categories_status='1' and (m.manufacturers_status='1' or m.manufacturers_status is null) and IF(p.products_bundle = 'no',p.products_quantity+p.store_quantity > '".(int)STOCK_MINIMUM_VALUE."',p.products_quantity > '".(int)STOCK_MINIMUM_VALUE."') and p.products_status='1' and p.is_store_item='0' and psv.value != '' "  .$price_str .  $man_str . $keywords_str.  $category_str. " order by psn.name, psv.value" );
	...
	//$specs_query = tep_db_query("select pa.options_id as specification_id, po.products_options_name as specification from products_attributes pa inner join products_options po on (pa.options_id=po.products_options_id and po.language_id='1') " . (!empty($category_filter) ? " inner join products_to_categories p2c on pa.products_id=p2c.products_id " : "") . " where (pa.options_values_id is null or pa.options_values_id <=0) " . (!empty($category_filter) ? " and p2c.categories_id='" . (int)$category_filter . "' " : "") . " group by pa.options_id order by po.products_options_name");
		...
		$specs_query = tep_db_query("select pa.options_id as specification_id, po.products_options_name as specification from products_attributes pa inner join products_options po on (pa.options_id=po.products_options_id)  inner join " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c on pa.products_id=p2c.products_id inner join products p on pa.products_id=p.products_id left join manufacturers m on p.manufacturers_id=m.manufacturers_id " . (!empty($keywords_str) ? " inner join products_description pd on p.products_id = pd.products_id and pd.language_id='1'" : "") . " where p.products_status='1' and (m.manufacturers_status='1' or m.manufacturers_status is null) and p.is_store_item='0' and IF(p.products_bundle = 'no',p.products_quantity+p.store_quantity > '".(int)STOCK_MINIMUM_VALUE."',p.products_quantity > '".(int)STOCK_MINIMUM_VALUE."') " . $category_str . $man_str . $keywords_str . " group by pa.options_id order by po.products_options_name");
	...
	while($entry = tep_db_fetch_array($specs_query)){
		...
		//$values_query = tep_db_query("select pa.options_values_id as value_id, pov.products_options_values_name as value_name from products_attributes pa inner join products_options_values pov on (pa.options_values_id=pov.products_options_values_id and pov.language_id='1' ) where pa.options_id='" . (int)$entry['specification_id'] . "' group by pa.options_values_id limit 0, 10");
            //$values_query = tep_db_query("select pa.options_values_id as value_id, pov.products_options_values_name as value_name from products_attributes pa inner join products_options_values pov on (pa.options_values_id=pov.products_options_values_id and pov.language_id='1') " . (!empty($category_filter) ? " inner join products_to_categories p2c on pa.products_id=p2c.products_id " : "") . " where pa.options_id='" . (int)$entry['specification_id'] . "' " . (!empty($category_filter) ? " and p2c.categories_id='" . (int)$category_filter . "' " : "") . " group by pa.options_values_id");
             $values_query = tep_db_query("select pa.options_values_id as value_id, pov.products_options_values_name as value_name from products_attributes pa inner join products_options_values pov on (pa.options_values_id=pov.products_options_values_id and pov.language_id='1')  inner join " . TABLE_PRODUCTS_TO_CATEGORIES . " p2c on pa.products_id=p2c.products_id inner join products p on pa.products_id=p.products_id left join manufacturers m on p.manufacturers_id=m.manufacturers_id ". (!empty($keywords_str) ? " inner join products_description pd on p.products_id = pd.products_id and pd.language_id='1'" : "") . " where pa.options_id='" . (int)$entry['specification_id'] . "' " . $category_str . $man_str . $keywords_str . (!empty($manufacturers_id) ? " and m.manufacturers_id='" . (int)$manufacturers_id . "' " : "") . " group by pa.options_values_id");
		...
		if (tep_db_num_rows($values_query)){
			...
			$response['options'][$entry['specification']] = array(
    ...
    while ($featured = tep_db_fetch_array($featured_query)) {
       ...
       if (tep_not_null($featured['products_image'])) {
            ...
            if ($feed_status && stripos($featured['products_largeimage'], 'http://')!==false){
                ...
                $image = '<img src="' . DIR_WS_IMAGES . "noimage.gif". '" title="' . $featured['products_name'] . '" class="img-responsive img-portfolio img-hover" style="width:350px;height:200px;">';
