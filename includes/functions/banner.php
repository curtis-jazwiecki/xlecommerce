...
// Update the banner click statistics
  ...
  // Check to see if a banner exists
  function tep_catbanner_exists($action, $cat_banners) {
  	if (sizeof($cat_banners) > 0) {
  	  $banners = implode(",", $cat_banners);
  	  $search = " and banners_id not in (" . $banners . ")";
  	  } else {
	   $search = '';
		}
      return tep_random_select("select banners_id, banners_title, banners_image, banners_html_text from " . TABLE_BANNERS . " where status = '1' and banners_group = 'category'" . $search);
