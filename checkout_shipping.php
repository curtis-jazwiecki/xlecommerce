   ...
   if (!tep_session_is_registered('comments'))
    ...
    if (SELECT_VENDOR_SHIPPING == 'true') {
		...
		foreach ($vendor_shipping as $vendor_id => $vendor_data) {
            ...
            // check for FFL licensee #start
            if ($method == 'ffldealershipping') {
                if (!array_key_exists($vendor_id, $_SESSION['ffl_selected'])) {
                    tep_redirect(tep_href_link(FILENAME_CHECKOUT_SHIPPING,'error_message=Select FFL Dealer', 'SSL'));
                }
            }
            // check for FFL licensee #ends
...
echo tep_image(DIR_WS_IMAGES . 'template/checkout_payment.gif', '', '100%',
    'auto');

?></td>
                <td background="images/template/checkout_bg.gif" style="background-repeat:repeat-x" class="image_progress_bar"><?php

echo tep_image(DIR_WS_IMAGES . 'template/checkout_confirmation.gif', '', '100%',
    'auto');

?></td>
                <td class="image_progress_bar"><?php

echo tep_image(DIR_WS_IMAGES . 'template/checkout_success.gif', '', '100%',
    'auto');
...
?></td>
        ...
        <tr>
          <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
              <tr>
                <td width="5" height="20" align="left" background="images/template/infoboxbg.jpg" class="heading_background_img"></td>
                <td class="infoBoxHeadingLogin" align="left"><b>
                  <?php
...
?>
                ...
                <td width="5" height="20" align="right" background="images/template/infoboxbg.jpg" class="heading_background_img new_img_heading"></td>
              </tr>
            </table></td>
        </tr>
        <tr>
	...
	if(MODULE_ORDER_TOTAL_AVATAX_VALIDATE != 'false'){
        ...
        function vaidateAvataxAddress(){
            ...
            jQuery.ajax({
                ...
                url: 'ava_api.php?mode=validateCheckoutAddressDetails',
				...
				data: 'sendto=' + encodeURIComponent(<?php echo $_SESSION['sendto']; ?>) + '&customer_id=' + encodeURIComponent(<?php echo $_SESSION['customer_id']; ?>),
                ...
                success: function(data) {
                    ...
                    if(data['error'] == 2){
						success_message = 'Error: This Country is not permissible for address validation!' + "\n\n";
...
?></td>
<td class="main" align="right"> <input type="radio" name="" id="" checked="checked" /></td>
                            <?php
...
<script type="text/javascript">
function showfflData(radio_buttons){
	jQuery('#ffl_dealers_iframe_'+radio_buttons).attr('src', jQuery('#ffl_dealers_iframe_'+radio_buttons).attr('src'));
	jQuery('#ffl_dealers_html_'+radio_buttons).show('slow');
}

<?php
if(isset($always_show) && $always_show != '') { ?>
	showfflData(<?php echo $always_show; ?>);
<?php } ?>

function hidefflData(radio_buttons){
	
	jQuery('#ffl_dealers_html_'+radio_buttons).hide('slow');
	
	jQuery.ajax({
		url: 'ffl_dealers.php',
		type: 'post',
		data: "action=removeDealersDetails&datay="+radio_buttons,
		dataType: 'html',
		beforeSend: function() {
		},
		success: function(response) {
			jQuery('#selected_ffl_'+radio_buttons).html('');
			//alert(response);
		}
	});
}
...
function pre_selected_ffl(x,y){
	jQuery.ajax({
		...
		success: function(response) {
			...
			setTimeout(function(){ 
				jQuery('#selected_ffl_'+y).html('<br>'+response);
				jQuery('#btn_select_'+y).hide('fast');
				jQuery('#selected_ffl_'+y).append('<br> <input type="button" onclick="showfflData('+y+')"  value="edit">');
			}, 1000);
...
<?php
		...
		pre_selected_ffl(<?php echo $fflID; ?>,<?php echo $vID; ?>);
	<?php
                    }
                }
?>
        ...
        <tr>
          <td><table border="0" width="100%" cellspacing="1" cellpadding="0">
              <tr class="infoBoxContents">
                <td><table border="0" width="100%" cellspacing="0" cellpadding="0">
                    <tr>
                      <td><div style="padding-right:5px">
                          <?php
