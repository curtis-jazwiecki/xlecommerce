...
// needs to be included earlier to set the success message in the messageStack
  ...
  $newsletter_query = tep_db_query("select customers_firstname, customers_lastname, customers_email_address, customers_newsletter from " . TABLE_CUSTOMERS . " where customers_id = '" . (int)$customer_id . "'");
  ...
  if (isset($HTTP_POST_VARS['action']) && ($HTTP_POST_VARS['action'] == 'process')) {
    ...
    if ($newsletter_general != $newsletter['customers_newsletter']) {
	  ...
	  if ($newsletter_general=='1'){
		tep_db_query("insert ignore into subscribers (email, name, is_registered) values ('" . tep_db_input($newsletter['customers_email_address']) . "', '" . tep_db_input($newsletter['customers_firstname'] . ' ' . $newsletter['customers_lastname']) . "', '1')");
	  } else {
		tep_db_query("delete from subscribers where email='" . tep_db_input($newsletter['customers_email_address']) . "'");
	  }
...
$sts->template['action_url'] = tep_href_link(FILENAME_ACCOUNT_NEWSLETTERS, '', 'SSL');
$sts->template['customers_newsletter'] = $newsletter['customers_newsletter'];
